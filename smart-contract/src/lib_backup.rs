...
impl AgroCoinContract {
    ...
    pub fn invest(env: Env, investor: Address, project_id: u64, amount: i128) {
        ...
        let mut existing_investment: Investment = env.storage()
            .persistent()
            .get(&investment_key)
            .unwrap_or(investment.clone());

        if existing_investment.amount > 0 {
            existing_investment.amount += amount;
        } else {
            existing_investment = investment;
        }

        env.storage().persistent().set(&investment_key, &existing_investment);
